...
* Preferably support for `VIRTUAL_HOST` environment variables for Docker
  containers. e.g. [`nginx-proxy`](https://github.com/nginx-proxy/nginx-proxy).
...
direct replacement for Docker for Mac, but is optimized to run much faster and
has built-in functionality for development domains.
...
![OSX preference pane providing access to VirtioFS](images/virtiofs.png)
...
### Bootstrap the local site

`task dev:reset` now starts the local environment with the latest published CMS
database snapshot image. That snapshot is taken immediately after
`drush site-install`, before the local post-install configuration is applied.

`task dev:reset` applies the post-install configuration after startup, so the
flow stays close to the previous local reset task. If you need to rerun the
post-install steps manually, use `task dev:cli -- ./dev-scripts/install-site.sh
--post-install-only`.

Use `task dev:reset:clean` when you need a full local reinstall, or when
working in BNF mode.
...
2. Validate that XDebug is enabled by inspecting <https://dpl-cms.local/admin/reports/status/php>.
...
1. To retrieve a database-dump from a running site, consult the
   ...
   database-dump.
2. Place the dump in the [cms/restore/database](../../cms/restore/database) directory, be aware
   that the directory is only allowed to contain a single `.sql` file.
3. Start a local environment using `task dev:reset:clean`
...
1. Place the files dump in the [cms/restore/files](../../cms/restore/files) directory, be aware
