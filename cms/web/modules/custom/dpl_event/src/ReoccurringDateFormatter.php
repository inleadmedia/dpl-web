...
class ReoccurringDateFormatter {
  ...
  /**
   * Finding the IDs of all events that have ended.
   *
   * @return array<int, string>
   *   An array of matching eventinstance IDs.
   */
  public function getPastEventIds(EventSeries $event_series): array {
    $date = new DrupalDateTime();
    $date->setTimezone(new \DateTimezone(DateTimeItemInterface::STORAGE_TIMEZONE));
    $formatted = $date->format(DateTimeItemInterface::DATETIME_STORAGE_FORMAT);
    ...
    return $query
      ...
      ->accessCheck(TRUE)
      ->condition('status', TRUE)
      ->sort('date.end_value', 'DESC')
      ->execute();
  }

  /**
   * Retrieves the most recent past event details for a given event series.
   *
   * @param \Drupal\recurring_events\Entity\EventSeries $event_series
   *   The event series object.
   *
   * @return null|array{'start': \Drupal\Core\Datetime\DrupalDateTime, 'end': \Drupal\Core\Datetime\DrupalDateTime, 'past_ids': array<int, string>}
   *   The most recent past event instance details, or NULL if none exist.
   */
  public function getPastEventDetails(EventSeries $event_series): array|null {
    $past_ids = $this->getPastEventIds($event_series);
    $past_event_id = reset($past_ids);

    if (!$past_event_id) {
      return NULL;
    }

    $event_instance = EventInstance::load($past_event_id);
