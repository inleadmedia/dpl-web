 ...
 *
 * DPL CMS imports `cms/config/sync` on every release. Configuration that only
 * exists in the site's active storage is treated as removed and deleted, so a
 * webmaster module that ships config through `config/install` loses it (and
 * its field data) on the next deployment. Adding our config to the import
 * storage makes the importer see it as part of the expected configuration.
 *
 * config/sync also holds our copy of DPL's search_api.index.content_events,
 * because editorial search adds fields and processors to that shared index.
 * That copy has to be refreshed whenever DPL changes the index upstream.
 *
 * @see https://github.com/danskernesdigitalebibliotek/dpl-web/blob/develop/docs/cms/webmaster-modules.md
 */
class OverlayConfigEventSubscriber implements EventSubscriberInterface {
  ...
  public function onImportTransform(StorageTransformEvent $event): void {
    $path = $this->pathResolver->getPath('module', 'eonext_editorial_search') . '/config/sync';
  ...
  public static function overlayConfig(StorageInterface $storage, StorageInterface $overlay): void {
    // getAllCollectionNames() does not include the default collection.
