...
use Drupal\bnf\GraphQL\Operations\GetNode\Node\Paragraphs\EmbedVideo\MediaVideotoolVertical;
...
trait EmbedVideoTrait {
   ...
   */
  public function getEmbedVideoValue(MediaVideo|MediaVideotool|MediaVideotoolVertical|ObjectLike|null $video): array {
    if (is_null($video)) {
      return [];
    }
    ...
    elseif (str_ends_with(get_class($video), 'MediaVideotoolVertical')) {
      $media = $this->getMediaVideoToolVerticalMedia($video, $mediaStorage);
    }
  ...
  /**
   * Finding or creating a VideoTool Vertical media.
   */
  private function getMediaVideoToolVerticalMedia(ObjectLike $video, EntityStorageInterface $mediaStorage): ?Media {
    /** @var \Drupal\bnf\GraphQL\Operations\GetNode\Node\Paragraphs\EmbedVideo\MediaVideotoolVertical $video */

    $properties = [
      'bundle' => 'videotool_vertical',
      'name' => $video->name ?? 'Video',
      'field_media_videotool_vertical' => $video->mediaVideotoolVertical,
      'status' => TRUE,
    ];
